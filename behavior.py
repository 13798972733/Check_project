...
import os
from classes.facenet.understand_facenet import detect_face
from classes.settings import FACE_DISTANCE_THRESHOLD
tf.reset_default_graph()
class ACNN(object):
    def __init__(self):
        #tf.reset_default_graph()
        self.drop_rate = 0.6                         # 丢弃率
        self.learning_rate = 0.00008                 # 学习率
        self.output_size = 4                    # 模型输出节点数
        self.batch_size = 100                 # 每次取n张图片                    # 模型输入图片大小
        self.model_save = "/home/itc/PycharmProjects/untitled/cws_AI/check/classes/object/modelsmaster/research/my_object_detection/models"
                  # 模型保存路径包括文件名
        self.data1 = []
        self.data = []                                      # 图像矩阵和标签
        ...
        self.CLASS_RESOURSE_PATH = "/home/itc/PycharmProjects/untitled/cws_AI/check/classes/object/modelsmaster/research/my_object_detection/images/3/1"
        self.CLASS_RESOURSE_PATH_ = "/home/itc/PycharmProjects/untitled/cws_AI/check/classes/object/modelsmaster/research/my_object_detection/images/3/2"
    ...
    def get_Batch(self, data, label, batch_size):
        input_queue = tf.train.slice_input_producer([data, label], num_epochs=1000, shuffle=True, capacity=32)
    ...
    def set_test_data(self):
        for class_id in os.listdir(self.CLASS_RESOURSE_PATH_):
            for stu_id in os.listdir(os.path.join(self.CLASS_RESOURSE_PATH, class_id)):
                path1 = os.path.join(self.CLASS_RESOURSE_PATH, class_id)
                img = Image.open(os.path.join(path1, stu_id))
                region = np.asarray(img)
